...
class Solution:
    ...
    def solve(self, row, n, board, result):
        ...
        if row == n:    # if all the queens are safely placed then add it to the solution
            result.append(self.addSolutions(board))
            return
        else:
    ...
    def isValid(self, row, col, board):
        ...
        return True
    # this function converts an array of boolean to an array of strings

    def addSolutions(self, board):
        solution = []   # saves the solution of each solution

        for row in board:                 # loops over every array in a list of arrays
            r = [ 'Q' if c else '.' for c in row] # loops over all the elements of an array
